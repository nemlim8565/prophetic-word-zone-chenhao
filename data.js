// data.js — 2026 年度 9 個關鍵領受資料
const REVELATIONS = [
  {
    id: "01",
    keyword: "洋蔥皮",
    subtitle: "Onion Skin",
    color: "#C8A97E",
    blocks: [
      {
        type: "narrative",
        content: "象徵著神正親手剝開老我的保護色與舊有的樣式\n呼召我們成為新造的人。"
      },
      {
        type: "attributes",
        content: "更新者\n脫去舊人\n內在根基"
      },
      {
        type: "prayer",
        content: "宣告聖靈要剝去那些為了自我保護\n而生出的「老我殼子」，求主賜下勇氣\n面對赤露敞開的生命更新。"
      },
      {
        type: "question",
        content: "1. 最近生活中有哪些「舊習慣」或「舊想法」是神正提醒你要像洋蔥皮一樣撥開的？\n2. 脫去舊人的過程通常會感到不安，你目前最需要神給你什麼樣的恩典來面對這個改變？"
      }
    ]
  },
  {
    id: "02",
    keyword: "切割好的田",
    subtitle: "Divided Field",
    color: "#E8734A",
    blocks: [
      {
        type: "narrative",
        content: "看見一塊被切割好的田地，其中一塊閃著紅光\n並且有農夫開始翻土。這象徵上帝即將在我們的生命中動工，特別是在那些令人心痛或不易的領域。"
      },
      {
        type: "attributes",
        content: "栽培者\n醫治的神\n內在根基"
      },
      {
        type: "prayer",
        content: "求主翻動我們生命中那些\n僵化、隱藏傷痛的「荒地」，主啊，我宣告\n有一翻土、更新的工作從祢而來，預備我們成為更肥沃的土壤，承載神國度的產業。"
      },
      {
        type: "question",
        content: "1. 你生命中是否有哪一塊區域正經歷「翻土」的時刻？你如何看待這份不容易？\n2. 翻土是為了更好的生長。你會期待神透過改善生命「土質」後，栽種什麼樣的新計畫？"
      }
    ]
  },
  {
    id: "03",
    keyword: "裸裝跑車",
    subtitle: "Naked Sports Car",
    color: "#6B9FD4",
    blocks: [
      {
        type: "narrative",
        content: "一台沒有外殼、正在升級引擎和關線的跑車\n象徵團隊的生命正在被神更新與預備，神正在此過程中\n不斷更新每一個人，如同跑車內的每個器件都在經歷更新。"
      },
      {
        type: "attributes",
        content: "耐心的神\n創始成終主\n內在根基"
      },
      {
        type: "prayer",
        content: "求主保守我們在「隱藏期」的耐心。不急於上路表現\n而是專注於內在生命（品格、情感）與基督元首的連結\n讓世人因我們的渴慕而相信你是超越一切的耶和華以羅伊。每一個人都是區裡重要的一員\n不論身處在哪個階段，你都值得被愛\n主求祢讓我們明白我們的受造是獨特且討你喜悅的。\n阿們。"
      },
      {
        type: "question",
        content: "1. 當看見身旁的人都在前進，而你感到自己仍處在「內部升級」時，你有什麼感受？你會如何面對?\n2. 在你目前生命光景中，有甚麼最需要被聖靈觸摸的地方?？"
      }
    ]
  },
  {
    id: "04",
    keyword: "萬花筒",
    subtitle: "Kaleidoscope",
    color: "#A855F7",
    blocks: [
      {
        type: "narrative",
        content: "萬花筒裡的視界藉著光就能看見各式各樣的形狀和顏色，像極了神所造的每一位\n不論現的生命情況、與上帝親近的程度、工作忙碌狀態\n。上帝的愛和恩典在基督裡都是是的"
      },
      {
        type: "attributes",
        content: "創造者\n眾光之父\n群體共榮"
      },
      {
        type: "prayer",
        content: "每一個人都是區裡重要的一員\n不論身處在哪個階段，你都值得被愛\n主求祢讓我們明白我們的受造是獨特且討你喜悅的。"
      },
      {
        type: "question",
        content: "1. 在小組中，你是否曾因覺得自己「不夠好」或「跟別人不一樣」而退縮？這個異象如何翻轉你的看法？\n2. 我們如何刻意創造一個環境，讓上帝的愛能透進我們的群體，展現萬花筒般的美麗？"
      }
    ]
  },
  {
    id: "05",
    keyword: "星星燈串",
    subtitle: "Star String",
    color: "#34D399",
    blocks: [
      {
        type: "narrative",
        content: "大小不一的星星構築成串，它們靠著一道電線串連通電而發出光芒。\n那道電線是神祂自己，象徵著「連結」的季節。\n沒有人是孤島。"
      },
      {
        type: "attributes",
        content: "連結者\n世界的光\n群體共榮"
      },
      {
        type: "prayer",
        content: "神要在2026看顧牧養更新調整幫助我們\n成為在各自生活領域靈命強壯的人\n對神的信賴與信心也會越發茁壯。"
      },
      {
        type: "question",
        content: "1. 你的環境(家庭、職場、小組)中有你感到困頓和受挑戰的地方嗎?你要如何在那裡散發屬神的光芒？\n2. 保持「通電」的關鍵是連結於神。這週你打算如何具體加強、維護與神的關係？"
      }
    ]
  },
  {
    id: "06",
    keyword: "攪動的池子",
    subtitle: "Stirring Pool",
    color: "#38BDF8",
    blocks: [
      {
        type: "narrative",
        content: "一個不斷往外擴張的池子，\n並且我看見有雙手在攪動這池子。\n神透過擴張我們的境界，帶領我們進入醫治的泉源"
      },
      {
        type: "attributes",
        content: "耶和華拉法\n醫治者\n群體共榮"
      },
      {
        type: "prayer",
        content: "神親自攪動這個池子，如同畢士大池\n祂正在做醫治的工作，特別是在關係、生命，與信仰之中與神的關係!"
      },
      {
        type: "question",
        content: "1. 異象中提到池子「往外推」，你認為神正帶領社青區進入哪些新的領域或人群？\n2. 在與神、與人的關係中，你是否正渴望一場「醫治的攪動」？讓我們一起為區裡的家人禱告。"
      }
    ]
  },
  {
    id: "07",
    keyword: "大能的勇士",
    subtitle: "Mighty Warrior",
    color: "#F59E0B",
    blocks: [
      {
        type: "narrative",
        content: "神在尋找真正的敬拜者——用心靈和誠實敬拜祂。\n2026 年的敬拜將進入一個新的深度，\n透過對於神慈愛和公義審判的認識，有一群大能的勇士會被神興起。"
      },
      {
        type: "attributes",
        content: "值得頌讚者\n萬王之王\n國度使命"
      },
      {
        type: "prayer",
        content: "呼求聖靈喚醒興傳講「真道」（包含審判與公義的福音）的勇士，不再做討好人的門徒\n勇士的甦醒始於認罪與悔改，向主俯伏跪拜。"
      },
      {
        type: "question",
        content: "1. 面對世界的價值觀，你是否曾為了「不得罪人」而選擇不談信仰？這份領受對你有什麼提醒？\n2. 一個勇士需要經歷破碎與認罪，你願意在此刻向主交託哪一件需要被主修剪的事？"
      }
    ]
  },
  {
    id: "08",
    keyword: "驢子進城",
    subtitle: "Entry on a Colt",
    color: "#8B5CF6",
    blocks: [
      {
        type: "narrative",
        content: "看見耶穌騎著驢子進城，那個驢子代表著人子要來服事世界\n是非常謙卑和捨己的樣式，祂甘願做在那些被世界看為最小的人們身上。"
      },
      {
        type: "attributes",
        content: "擴張\n謙卑君王\n國度使命"
      },
      {
        type: "prayer",
        content: "聖靈，求祢賜下如耶穌般「降卑」的心志\n更是擴張我們的心，讓我們樂意服事那些最小的、下一代與不被看見的群體。"
      },
      {
        type: "question",
        content: "1. 在競爭激烈的職場生活中，要持守耶穌基督的謙卑，最大的挑戰是什麼？\n2. 誰是你生活圈中「最小的那一位」？這週你可以為他做一件什麼樣具體的服事？"
      }
    ]
  },
  {
    id: "09",
    keyword: "只在乎祢",
    subtitle: "Only the Lord",
    color: "#D97706",
    blocks: [
      {
        type: "narrative",
        content: "生命動力的轉向，將焦點從「人的評價」轉向「神的榮耀」。\n因為只在乎神，才能在軟弱中誇口，抵擋世界誘惑。"
      },
      {
        type: "attributes",
        content: "建造者\n獨一真神\n國度使命"
      },
      {
        type: "prayer",
        content: "因為凡事都看見神，所以去愛人,\n所以去服事，所以忍耐，所以謙卑的生命\n禱告有一份成熟的愛要在我們的心中長出來。"
      },
      {
        type: "question",
        content: "1. 想像一下，如果你真的「只在乎神」的評價，你現在的工作與生活態度會有什麼轉變？\n2. 在軟弱中誇口（林後12:9）對你而言具體代表什麼？你如何在軟弱中看見神的能力？"
      }
    ]
  }
];
